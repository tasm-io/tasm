...
Typescript Asssembly Simulator (TASM) is an web-based simulator for a simple microprocessor. The system is aimed at students learning assembly language, or mature users curious about how assembly language works. It runs on any (modern) web browser, and focuses on ease of use and accessibility to ensure that it is available to a diverse a group of people as possible. TASM provides a number of useful features that aim to make it more interactive and enjoyable for learners. The primary features include:  
...
- *Semantic Checker*: This component performs a number of passes over the syntax tree generated by the preceeding component, and each pass looks for errors, e.g. undefined identifiers. The purpose of this pass is to detect mistakes made by the user earlier on so that the code for the latter components is simpler, and better error messages are produced.
...
- *Code generator*: This component takes the tree outputted by the preceeding component, and transforms it into executable bytecode. Because we put a lot of work into minifying and checking our syntax tree in the previous passes, this component is actually very simple.
